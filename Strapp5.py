...
def build_instance_id(gamme_id, op_id, ordre):
    return f"{gamme_id}_{op_id}_{ordre}"

def get_operations_selection(game_id):
    """Step 4: Operations selection (FIXED & STABLE)"""
    ...
    for op in operations:
        op_id = op.get("id_operation")
        if op_id not in st.session_state.op_selection:
            st.session_state.op_selection[op_id] = True  # default: ALL selected
    ...
    with col1:
        if st.button("✅ Tout sélectionner", width="stretch"):
            for k in st.session_state.op_selection:
                st.session_state.op_selection[k] = True

    with col2:
        if st.button("🔄 Inverser la sélection", width="stretch"):
            for k in st.session_state.op_selection:
                st.session_state.op_selection[k] = not st.session_state.op_selection[k]

    with col3:
        if st.button("❌ Supprimer non sélectionnées", width="stretch"):
            st.session_state.op_selection = {
                k: v for k, v in st.session_state.op_selection.items() if v
            }
    ...
    for op in operations:
        ...
        with cols[0]:
            st.session_state.op_selection[op_id] = st.checkbox(
                "",
                value=st.session_state.op_selection[op_id],
                key=f"op_chk_{game_id}_{op_id}_{ordre}"
            )
    ...
    # ---- FINAL SELECTED LIST ----
    selected_ops = [
        op_id for op_id, selected in st.session_state.op_selection.items() if selected
    ]
